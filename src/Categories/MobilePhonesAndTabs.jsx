...
const MobilePhonesAndTabs = () => {
  ...
  const getImageUrl = (path) => {
    ...
    return path ? `https://kara-backend-1.onrender.com${path}` : fallbackImage;
  };
  ...
  const fetchProducts = async () => {
    try {
      ...
      const response = await fetch(
        `https://kara-backend-1.onrender.com/products/public?category=${encodeURIComponent(
          category
        )}`,
        {
          headers,
      ...
      console.log(
        `${displayCategory} Products: Response status:`,
        response.status,
        response.statusText
      );
      const data = await response.json();
      console.log(
        `${displayCategory} Products: Raw response:`,
        JSON.stringify(data, null, 2)
      );
      if (!response.ok)
        throw new Error(
          data.message || `Failed to fetch ${displayCategory} products`
        );
          ...
          <div className="grid grid-cols-3 gap-6">
            {products.length > 0 ? (
              ...
              <p className="text-gray-600">
                No {displayCategory} products available.
              </p>
            )}
          </div>
        </div>
